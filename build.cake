...
var product = "Htc.Vita.Wix.BalExtension";
var productCustomBootstrapper = "Htc.Vita.Wix.StdBA";
var productCustomLibrary = "Htc.Vita.Wix.BalLibrary";
var companyName = "HTC";
var version = "0.9.0";
...
var customBootstrapperProjectFile = File($"./source/{productCustomBootstrapper}/{productCustomBootstrapper}.vcxproj");
var customLibraryProjectFile = File($"./source/{productCustomLibrary}/{productCustomLibrary}.wixproj");
var extensionProjectFile = File($"./source/{product}/{product}.net20.csproj");
...
Task("Build-Wix-Custom-Bootstrapper")
    .IsDependentOn("Generate-AssemblyInfo")
    .Does(() =>
{
    // Use MSBuild
    MSBuild(
            customBootstrapperProjectFile,
            settings => settings.SetConfiguration(configuration)
    );
});

Task("Sign-Wix-Custom-Bootstrapper")
    .WithCriteria(() => "Release".Equals(configuration) && !"NOTSET".Equals(signPass) && !"NOTSET".Equals(signKeyEnc))
    .IsDependentOn("Build-Wix-Custom-Bootstrapper")
    .Does(() =>
{
    ...
    var file = $"./temp/{configuration}/wixstdba/bin/wixstdba.dll";
...
Task("Build-Wix-Custom-Library")
    .IsDependentOn("Sign-Wix-Custom-Bootstrapper")
    .Does(() =>
{
    // Use MSBuild
    MSBuild(
    ...
    .Does(() =>
{
    ...
    var file = $"./temp/{configuration}/{product}/bin/net20/{product}.dll";
    if (totalTimeInMilli < signIntervalInMilli)
    {
        System.Threading.Thread.Sleep(signIntervalInMilli - (int)totalTimeInMilli);
    }
    ...
    .Does(() =>
{
    ...
    var nuGetPackSettings = new NuGetPackSettings
    {
            ...
            Files = new []
            {
                    new NuSpecContent
                    {
                            Source = $"{product}/bin/net20/{product}.dll",
                            Target = "tools\\lib"
                    },
                    new NuSpecContent
                    {
                            Source = $"{product}/bin/net20/{product}.pdb",
                            Target = "tools\\lib"
                    },
            },
